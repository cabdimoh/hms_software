$(document).ready(function () {
  ...
  $("#create_new_groupForm").on("submit", function (e) {
    ...
    if (insertType == "Update") {
      ...
      method = "POST";
      link = "/users/rename_group";
    } else {
      method = "POST";
      link = "/users/manage_group/0";
    }
    if (name != "") {
      $.ajax({
        method: method,
      ...
      Swal.fire("warning", "Enter group name", "error");
    }
  });
  ...
  $("#group_permissions tbody").on("click", ".editPerms", function (e) {
    let id = $(this).attr("GroupID");
    window.location.replace("/users/edit_group/" + id);
  });
  ...
  function showGroup(id) {
    $.ajax({
      method: "GET",
      url: "/users/manage_group/" + id.toString(),
      headers: { "X-CSRFToken": csrftoken },
      async: false,
      success: function (data) {
        if (!data.isError) {
  ...
  $("#group_permissions tbody").on("click", ".deletGroup", function (e) {
    ...
    Swal.fire({
      ...
      if (e.value) {
        $.ajax({
          method: "DELETE",
          url: "/users/manage_group/" + GroupID.toString(),
          headers: { "X-CSRFToken": csrftoken },
          async: false,
          success: function (data) {
            if (!data.isError) {
