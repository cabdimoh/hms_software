$(document).ready(function () {
  ...
  function GetReport() {
    ...
    $.ajax({
      ...
      async: false,
      success: function (data) {
        let table = $("#usersList").DataTable().clear().draw();

        data.Message.forEach((item) => {
          table.row
            .add([
              `<span class="avatar avatar-md brround" style="background-position:center; background-size: cover; background-image: url(/media/${item.Avatar})"></span>`,
              item.Username,
              item.Name,
            ])
            .draw();
